...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`📧 Proxy MARCAS: Obteniendo correos para marcas@ - límite: ${limit}, fecha: ${dateFrom}`)

    // Construir URL para el servidor Python
    const pythonUrl = new URL('/api/emails/for-marcas', PYTHON_SERVER_URL)
    pythonUrl.searchParams.set('limit', limit.toString())
    if (dateFrom) {
      pythonUrl.searchParams.set('date_from', dateFrom)
    }
    
    console.log(`🔗 Proxy MARCAS: Llamando a ${pythonUrl.toString()}`)
    ...
    console.log(`✅ Proxy MARCAS: Encontrados ${data.total_found} correos, mostrando ${data.showing}`)
    ...
    const transformedEmails = (data.emails || []).map((email: any, index: number) => ({
      ...
      filter_applied: 'marcas@patriciastocker.com'
    }))
    ...
    console.error('❌ Error en proxy de correos para marcas:', error)
    
    return NextResponse.json({
      error: error instanceof Error ? error.message : 'Error obteniendo correos para marcas',
      emails: [],
      total_found: 0,
      showing: 0,
      filter: 'marcas@patriciastocker.com',
